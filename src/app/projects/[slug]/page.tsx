  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-4xl mx-auto px-6 py-24">
        <Link href="/projects" className="text-purple-400 hover:text-purple-300 text-sm mb-8 inline-block">{t.backProjects}</Link>
        ...
        <div className="flex gap-4 mb-12 flex-wrap">
          {doc.meta.status && <span className="text-xs px-3 py-1 rounded bg-green-500/10 text-green-300">{doc.meta.status}</span>}
          {doc.meta.tags?.map((tag: string) => (<span key={tag} className="text-xs px-3 py-1 rounded bg-purple-500/10 text-purple-300">{tag}</span>))}
        </div>
